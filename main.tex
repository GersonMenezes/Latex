...
\title{Plataforma Híbrida de Instrumentação para biossinais: Redução de Ruído com Hardware de Precisão e Inteligência Artificial}
...
% -------------------------------------------------------------------------
\begin{abstract}
  A aquisição de biossinais, como o Eletrocardiograma (ECG) e a Eletromiografia (EMG), é fundamental na saúde e na pesquisa científica, pois fornece dados cruciais para o diagnóstico clínico de patologias, o monitoramento contínuo de pacientes e o desenvolvimento de interfaces homem-máquina. No entanto, devido às suas características intrínsecas de baixíssima amplitude, esses sinais são altamente suscetíveis a ruídos da rede elétrica e artefatos de movimento. Equipamentos profissionais de alta fidelidade possuem custo elevado, enquanto soluções de baixo custo carecem de precisão. Este trabalho propõe o desenvolvimento de uma plataforma híbrida de instrumentação para a aquisição e análise de biossinais, combinando um hardware de alta precisão com pós-processamento via \textit{Deep Learning}. A abordagem se baseia em duas frentes de atuação: primeiramente, o projeto de um circuito de condicionamento analógico de baixo ruído, responsável por amplificar e pré-filtrar o sinal na sua origem. Em seguida, os dados adquiridos são processados em um computador por um algoritmo de aprendizado profundo, que atua como uma segunda camada de filtragem adaptativa para atenuar ruídos complexos. Destaca-se que a arquitetura desenvolvida é modular, podendo ser facilmente expandida e adaptada para a aquisição de outros tipos de biossinais. Como principal contribuição, espera-se obter um protótipo funcional de baixo custo que demonstre uma melhora significativa na relação sinal-ruído, validado através da aquisição de sinais reais de ECG e EMG.
\end{abstract}

% -------------------------------------------------------------------------
% ABSTRACT (INGLÊS)
% -------------------------------------------------------------------------
\begin{englishabstract}{Hybrid Instrumentation Platform for Biosignals: Noise Reduction with Precision Hardware and Deep Learning}
  The acquisition of biosignals, such as Electrocardiogram (ECG) and Electromyography (EMG), is fundamental in healthcare and scientific research, as it provides crucial data for the clinical diagnosis of pathologies, continuous patient monitoring, and the development of human-machine interfaces. However, due to their intrinsic low-amplitude characteristics, these signals are highly susceptible to power line noise and motion artifacts. High-fidelity professional equipment is expensive, while low-cost solutions lack precision. This work proposes the development of a hybrid instrumentation platform for the acquisition and analysis of biosignals, combining high-precision hardware with post-processing via Deep Learning. The approach is based on two fronts: firstly, the design of a low-noise analog conditioning circuit, responsible for amplifying and pre-filtering the signal at its source. Next, the acquired data is processed on a computer by a deep learning algorithm, acting as a second layer of adaptive filtering to attenuate complex noises. It is noteworthy that the developed architecture is modular and can be easily expanded and adapted for the acquisition of other types of biosignals. As a main contribution, a functional, low-cost prototype demonstrating a significant improvement in the signal-to-noise ratio is expected, validated through the acquisition of real ECG and EMG signals.
\end{englishabstract}
...
A instrumentação biomédica dedica-se ao desenvolvimento de sistemas para medir e monitorar sinais fisiológicos. Dentre estes, os biopotenciais --- sinais elétricos gerados por atividade celular, como o Eletrocardiograma (ECG) e a Eletromiografia (EMG) --- são dos mais estudados e clinicamente relevantes. A aquisição de tais sinais tem como finalidade primária fornecer dados cruciais para o diagnóstico de patologias (como arritmias cardíacas no caso do ECG, ou disfunções neuromusculares no caso da EMG), além de viabilizar o monitoramento contínuo de pacientes e o desenvolvimento de interfaces homem-máquina na pesquisa em reabilitação. Historicamente, a aquisição destes sinais era restrita a ambientes clínicos controlados, utilizando equipamentos analógicos volumosos e de alto custo \cite{goldberger2000}.

Com o avanço da microeletrônica, tornou-se viável o desenvolvimento de sistemas de aquisição de dados (DAQ) portáteis e de baixo custo, frequentemente baseados em microcontroladores. Plataformas como o Arduino popularizaram o acesso a essas tecnologias. No entanto, a aquisição de biossinais apresenta um desafio de engenharia significativo: os sinais de interesse possuem amplitude muito baixa (na faixa de microvolts a milivolts) e seu espectro de frequência se sobrepõe a diversas fontes de ruído, notadamente o ruído da rede elétrica (60 Hz e seus harmônicos) e artefatos de movimento \cite{pathak2025}. Estes artefatos caracterizam-se como distorções espúrias no sinal, causadas por alterações temporárias na impedância da interface pele-eletrodo durante a movimentação física do paciente, manifestando-se como flutuações na linha de base ou picos repentinos de saturação.
...
Este trabalho se justifica pela lacuna existente entre os sistemas de instrumentação de baixo custo e a necessidade de alta fidelidade na aquisição de biossinais. A inovação do projeto reside na abordagem híbrida: em vez de depender unicamente do hardware ou do software, propõe-se uma sinergia entre ambos. O projeto se distingue de outros por não tentar corrigir um sinal de baixa qualidade (adquirido por hardware genérico) apenas com processamento digital. A hipótese é que, ao projetar um hardware de precisão dedicado (um circuito analógico de baixo ruído), o sinal entregue ao processador será de qualidade significativamente superior, permitindo que a camada de \textit{Deep Learning} atue de forma mais fina e eficaz. Ademais, a arquitetura desenvolvida oferece grande versatilidade; uma vez testada e validada para as frequências e amplitudes do ECG e da EMG, a plataforma poderá ser facilmente adaptada para a captura de sinais variados que apresentem características elétricas semelhantes, ampliando o seu impacto na pesquisa em instrumentação biomédica.

\section{Objetivos}

\subsection{Objetivo Geral}
Desenvolver e validar uma plataforma híbrida de instrumentação para aquisição de biossinais (ECG e EMG), que utilize a sinergia entre um \textit{shield} de condicionamento analógico de precisão e algoritmos de Inteligência Artificial para maximizar a relação sinal-ruído.

\subsection{Objetivos Específicos}
\begin{itemize}
    \item Realizar um estudo aprofundado sobre a natureza dos biossinais, técnicas de condicionamento analógico e métodos de IA;
...
% -------------------------------------------------------------------------
\chapter{Aquisição e Processamento de biossinais}
...
Dentre as interferências mais críticas, destaca-se a Interferência da Rede Elétrica (IRE), que no Brasil opera em 60 Hz. Esse ruído acopla-se ao corpo do paciente e aos cabos de aquisição por meio de acoplamentos capacitivos e indutivos, manifestando-se como uma senóide persistente que pode mascarar completamente ondas de baixa amplitude, como a onda P no ECG. Outro desafio onipresente são os artefatos de movimento, originados pela variação da impedância na interface pele-eletrodo e pela movimentação dos cabos (efeito triboelétrico). Esses artefatos provocam a chamada flutuação da linha de base (baseline wander), tipicamente em frequências abaixo de 1 Hz, causadas pela respiração e mudanças posturais, além de picos de saturação que mimetizam arritmias ou contrações musculares espúrias \cite{marchetti2006, balbinot2019v1}.

A complexidade da filtragem reside no fato de que o espectro de frequência desses ruídos frequentemente se sobrepõe ao espectro do sinal biológico. Por exemplo, enquanto o complexo QRS do ECG possui componentes importantes até 100 Hz, o ruído de 60 Hz e as interferências eletromiográficas de outros músculos (ruído de crosstalk) ocupam essa mesma faixa. O uso de filtros analógicos ou digitais convencionais (como filtros Notch ou Passa-Baixas agressivos) impõe um dilema: uma filtragem muito estreita pode causar distorções de fase e atenuação de componentes vitais do sinal, alterando a morfologia das ondas e prejudicando o diagnóstico clínico. Essa natureza não estacionária e a sobreposição espectral dos ruídos justificam a necessidade de abordagens mais robustas, que combinem hardware de alta precisão para a rejeição de modo comum com algoritmos inteligentes capazes de distinguir a morfologia do sinal biológico do padrão de ruído \cite{pathak2025, goel2013}.
...
A padronização do registro de ECG é essencial para a análise clínica. O sinal é tradicionalmente impresso sobre uma grade milimetrada padrão, onde cada eixo possui um significado físico preciso. No eixo horizontal, que representa o tempo, a velocidade padrão de registro é de 25 mm/s; logo, cada milímetro corresponde a 40 ms e cada bloco de 5 mm equivale a 200 ms. Simultaneamente, o eixo vertical representa a amplitude do sinal elétrico com uma calibração padrão de 10 mm/mV. Desta forma, cada subdivisão menor (1 mm) reflete uma variação de 0,1 mV, e cada bloco maior (5 mm) totaliza 0,5 mV. Essa padronização permite que médicos e sistemas automatizados mensurem visualmente a frequência cardíaca e a duração de intervalos críticos, como o intervalo PR e o segmento ST.

A análise clínica do ECG baseia-se não apenas na presença das ondas, mas na duração e amplitude dos intervalos entre elas. Segundo \citet{souto2016} e ilustrado na Figura \ref{fig:ecg}, a atividade elétrica cardíaca normal segue um padrão hierárquico e temporal rigoroso:
...
Para a aquisição do sinal, comumente utilizam-se eletrodos de superfície (sEMG), que são dispositivos não invasivos aderidos à pele. A interface eletrodo-pele atua como um transdutor que capta a corrente iônica gerada nos músculos e a converte em corrente eletrônica para o sistema de instrumentação. Segundo \citet{marchetti2006}, a configuração bipolar é a mais adequada para esse tipo de aplicação. Nela, dois eletrodos de detecção são posicionados sobre o ventre muscular e um terceiro eletrodo (referência) é colocado em uma região eletricamente neutra. Essa configuração permite o uso de amplificação diferencial, fundamental para eliminar ruídos de modo comum que afetam ambos os eletrodos de detecção.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{imagens/eletrodo_illustration.png}
    \caption{Aquisição de biossinais utilizando-se eletrodos de superfícies}
    \label{fig:semg}
\end{figure}
...
O estágio de instrumentação e condicionamento analógico atua como a ponte entre o fenômeno biológico e o mundo digital. Devido à fragilidade dos biossinais, não é viável conectá-los diretamente a um processador; é necessário um circuito intermediário (\textit{front-end} analógico) capaz de adequar as características elétricas do sinal. As subseções a seguir detalham os três pilares fundamentais desse processo: amplificação, filtragem e conversão analógico-digital.
...
A taxa de amostragem deve obedecer ao Teorema de Nyquist, sendo pelo menos o dobro da maior frequência contida no sinal para evitar perdas de informação. Já a resolução, medida em bits, define a "granularidade" da medida. Microcontroladores comuns (como o Arduino Uno) tipicamente possuem ADCs de 10 bits, o que divide a faixa de leitura em apenas 1024 níveis. Para aplicações de biossinais de alta precisão, onde micro-variações são diagnósticas, recomenda-se o uso de ADCs externos de maior resolução (como 16 bits ou superior), permitindo detectar nuances do sinal que seriam perdidas em quantizações mais grosseiras \cite{balbinot2019v1}.
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
      block/.style={rectangle, draw, thick, text width=3.5cm, text centered, rounded corners, minimum height=2cm, font=\small},
      line/.style={draw, thick, -latex}
  ]
      % Definindo os nós (blocos)
      % Aumentado o distanciamento de 1.5cm para 2.5cm
      \node [block, fill=blue!10] (paciente) {Paciente\\(Eletrodos de Superfície)};
      \node [block, right=2.5cm of paciente, fill=orange!10] (front) {\textbf{Bloco 1}\\Front-End Analógico\\(AD620 + OPA2134)};
      \node [block, right=2.5cm of front, fill=green!10] (adc) {\textbf{Bloco 2}\\Conversão e Controle\\(ADS1115 + Arduino)};
      \node [block, below=1.5cm of adc, fill=purple!10] (pc) {\textbf{Blocos 3 e 4}\\Comunicação, IHM e \textit{Deep Learning} (PC)};
  
      % Desenhando as setas de conexão com deslocamento vertical (yshift)
      \path [line] (paciente) -- node[above, yshift=0.1cm, font=\footnotesize] {Biopotencial} (front);
      \path [line] (front) -- node[above, yshift=0.1cm, font=\footnotesize] {Sinal Filtrado} (adc);
      \path [line] (adc) -- node[right, xshift=0.1cm, font=\footnotesize] {USB/Serial} (pc);
  
  \end{tikzpicture}
  \caption{Diagrama de blocos geral da plataforma híbrida de instrumentação, ilustrando o fluxo do sinal desde a aquisição analógica até o processamento computacional.}
...
\subsection{Detalhamento do Front-End Analógico (Bloco 1)}O \textit{front-end} analógico é composto por um amplificador de instrumentação (AD620) e um amplificador operacional duplo (OPA2134). O AD620 atua na amplificação diferencial inicial, sendo um componente de baixo ruído e alta precisão, ideal para a aquisição de biopotenciais de baixa amplitude. O OPA2134, também caracterizado por seu baixo ruído, é utilizado em cascata para compor os estágios de filtragem ativa. O circuito é alimentado de forma simétrica com +9V (VCC) e -9V (VEE), tensões estas provenientes de duas baterias de 9V em série. 

\begin{figure}[htpb]
  \centering
  \includegraphics[width=\textwidth]{imagens/bloco1.pdf}
  \caption{Diagrama esquemático do circuito de aquisição e filtragem analógica.}
  \label{fig:esquematico_bloco1}
\end{figure}

\subsubsection{Estágio de Amplificação Inicial}A primeira etapa do condicionamento consiste na amplificação do biossinal através do AD620. O ganho ($G$) deste componente é configurado por um único resistor externo ($R_g$, denotado como $R_1$ no diagrama esquemático), governado pela equação fornecida pelo fabricante:$$G = 1 + \frac{49400}{R_g}$$Para a aquisição de biopotenciais, estabeleceu-se um ganho inicial na casa de 100. Este valor foi projetado para amplificar o sinal de forma significativa sem que eventuais flutuações da linha de base causem a saturação prematura do \textit{chip} antes da etapa de filtragem. Utilizando um resistor de precisão comercial $R_1 = 510 \Omega$, obtém-se um ganho prático de aproximadamente 98.

\subsubsection{Dimensionamento dos Filtros Ativos}Para garantir a integridade do sinal, eliminando o componente DC e ruídos de alta frequência antes da digitalização, foram implementados filtros ativos de segunda ordem na topologia Sallen-Key. Para facilitar o projeto e manter a resposta em frequência com a curva plana característica dos filtros Butterworth, igualaram-se os pares de componentes resistivos e capacitivos em cada estágio. A frequência de corte ($f_c$) é definida pela equação:$$f_c = \frac{1}{2\pi R C}$$O primeiro filtro da cascata é um \textbf{Passa-Alta Ativo (U2A)}, projetado para eliminar a flutuação de linha de base causada predominantemente pela respiração do paciente. Para atingir a frequência de corte alvo de 0,5 Hz, os componentes foram dimensionados com $C_1 = C_2 = 1 \mu\text{F}$ e $R_2 = R_3 = 330 \text{ k}\Omega$.O estágio subsequente é um \textbf{Filtro Passa-Baixa Ativo (U2B)}, cuja função é atenuar interferências e ruídos de altas frequências indesejadas que se sobrepõem ao sinal de ECG. Este estágio foi dimensionado com os capacitores $C_3 = C_4 = 100 \text{ nF}$ e resistores $R_4 = R_5 = 15 \text{ k}\Omega$. Aplicando estes valores na fórmula, obtém-se uma frequência de corte teórica de aproximadamente 106 Hz. Este limite superior atua de forma precisa e eficiente para englobar a banda de frequências do complexo QRS do coração humano.

\subsection{Conversão e Controle (Bloco 2)}

O sinal analógico condicionado pelo circuito \textit{front-end} é encaminhado para o conversor analógico-digital ADS1115 (pino AIN0). A escolha deste componente se justifica pela sua resolução de 16-bits, que oferece uma precisão substancialmente maior na quantização dos biossinais quando comparada aos conversores nativos de 10-bits presentes no microcontrolador, permitindo detectar micro-variações no sinal filtrado.

O ADS1115 comunica-se com o microcontrolador (Arduino Uno) por meio do protocolo bidirecional I$^2$C, utilizando os pinos SDA (\textit{Serial Data}) e SCL (\textit{Serial Clock}). Como este protocolo opera em arquitetura de dreno aberto (\textit{open-drain}), foram dimensionados resistores de \textit{pull-up} de 10 k$\Omega$ em ambas as linhas, conectados à alimentação de +5V. Isso garante que o barramento retorne ao nível lógico alto (1) quando estiver ocioso, estabilizando a comunicação digital. O pino de endereço (ADDR) foi aterrado, definindo o endereço I$^2$C padrão do escravo como \texttt{0x48}.

A Figura \ref{fig:esquematico_bloco2} ilustra a topologia do circuito de conversão e comunicação digital.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imagens/bloco2.pdf}
    \caption{Diagrama esquemático do bloco de conversão A/D e controle digital.}
    \label{fig:esquematico_bloco2}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Comparativo de Filtragem Digital. A) Sinal de entrada contaminado por ruído de rede de 60 Hz. 
  B) Sinal de saída após aplicação do filtro IIR Notch em tempo real, recuperando a morfologia do ECG.
  C) Sinal de saída após aplicação do filtro DCAE em tempo real, recuperando a morfologia do ECG.}
...
\subsection{Limitações da Filtragem Clássica e a Abordagem Híbrida}

Conforme demonstrado na etapa anterior, filtros digitais tradicionais, como o IIR Notch, são extremamente eficientes e computacionalmente leves para eliminar interferências estacionárias e de frequência bem definida, a exemplo do ruído da rede elétrica (60 Hz). No entanto, a aquisição de biossinais no mundo real está sujeita a artefatos complexos e caóticos, como o ruído eletromiográfico (contração de outros músculos) e a flutuação da linha de base gerada pela respiração ou movimentação do paciente. 

O grande desafio reside no fato de que o espectro de frequência desses artefatos frequentemente se sobrepõe à faixa de frequência do sinal cardíaco (0,5 a 100 Hz). A aplicação de filtros clássicos agressivos para limpar esses ruídos invariavelmente causa distorções de fase temporal, atenuando as ondas P e T e alterando a morfologia clínica do ECG. Para contornar essa limitação sem onerar o hardware de aquisição, implementou-se uma segunda camada de pós-processamento utilizando Inteligência Artificial, especificamente o \textit{Deep Learning}. A premissa é que, ao invés de atuar isolando frequências, o modelo aprenda a reconstruir a morfologia estrutural do batimento cardíaco, descartando ruídos não-estacionários.

\subsection{Base de Dados e Estruturação do \textit{Dataset}}

Para que a rede neural aprendesse a isolar as características fisiológicas do coração, foi necessário treiná-la com um volume robusto de sinais de alta fidelidade. Optou-se pela utilização do banco de dados \textit{PTB Diagnostic ECG Database} (PTBDB), disponibilizado pelo PhysioNet, devido a duas características fundamentais para o alinhamento com o hardware do projeto: a frequência de amostragem de 1000 Hz e a disponibilidade da derivação D1 (Lead I), que reflete perfeitamente a diferença de potencial entre os membros superiores captada pelo Amplificador de Instrumentação (INA) da plataforma. O banco PTBDB também agrega diversidade clínica (pacientes saudáveis, infartados, com arritmias, etc.), garantindo que a rede aprenda a preservar anomalias cardíacas reais em vez de apagá-las como ruído.

O pré-processamento dos sinais brutos (\texttt{.dat} e \texttt{.hea}) foi conduzido na linguagem Python, utilizando a biblioteca oficial \texttt{wfdb} (\textit{Waveform Database}). Como o volume de dados ultrapassa a capacidade de memória padrão (549 registros), a estratégia adotada baseou-se na classe \texttt{Dataset} nativa do \textit{framework} PyTorch. A rotina foi programada para fatiar o sinal contínuo em janelas curtas de 2 segundos (equivalente a 2000 amostras por janela) no momento exato em que a rede solicita o dado para treinamento.

Nesta etapa, aplicou-se a técnica de \textit{Data Augmentation}. A cada iteração, o algoritmo recorta a janela limpa e injeta, de forma estocástica (com amplitudes e frequências variáveis), uma combinação de ruído de rede (60 Hz), flutuação de linha de base e ruído branco gaussiano. Ao entregar pares dinâmicos de sinais "sujos" na entrada e seus respectivos gabaritos "limpos" na saída, impede-se que o modelo decore os dados (overfitting), forçando-o a generalizar o aprendizado e focar estritamente na forma do pulso cardíaco.

\subsection{Arquitetura do Autoencoder Convolucional (DCAE)}

A arquitetura do filtro inteligente foi baseada em um \textit{Denoising Convolutional Autoencoder} (DCAE) unidimensional, estruturado em duas etapas principais que mapeiam as matrizes no formato \texttt{[Lote, Canais, Amostras]}:

\begin{itemize}
    \item \textbf{\textit{Encoder} (Codificador):} Responsável por extrair as características do sinal e comprimir a informação. Foram utilizadas camadas de convolução (\texttt{Conv1d}) combinadas com funções de ativação \texttt{ReLU} e subamostragem (\texttt{MaxPool1d}). Partindo da janela original de 2000 pontos, o sinal passa por duas etapas de compressão temporal, alcançando um "gargalo" (espaço latente) de 500 pontos. Essa compressão obriga a rede a reter apenas os padrões estruturais repetitivos do ECG, pois os artefatos de ruído são imprevisíveis e difíceis de serem codificados nesse espaço reduzido.
    \item \textbf{\textit{Decoder} (Decodificador):} Realiza o caminho inverso para recuperar a dimensionalidade original e redesenhar a onda. Durante os testes preliminares, a utilização de camadas \texttt{ConvTranspose1d} gerou oscilações de alta frequência no sinal de saída, um fenômeno conhecido na literatura de \textit{Deep Learning} como Artefato de Tabuleiro de Xadrez (\textit{Checkerboard Artifact}), causado pela sobreposição excessiva dos \textit{kernels} na etapa de expansão. Para solucionar esse problema, a topologia do \textit{Decoder} foi alterada para utilizar blocos de interpolação linear (\texttt{Upsample}) seguidos por convoluções simples (\texttt{Conv1d}). O \textit{Upsample} estica o sinal suavemente e a convolução atua refinando as arestas, entregando um ECG liso e com dimensões restauradas (2000 pontos).
\end{itemize}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{imagens/filtro_DCAE.png}
  \caption{Comparativo de Filtragem Digital. 
  A) Sinal original. 
  B) Sinal contaminado..
  C) Sinal de saída após aplicação do filtro DCAE em tempo real, recuperando a morfologia do ECG.}
  \label{fig:filtro_dcae}
\end{figure}

\subsection{Treinamento e Otimização do Modelo}

A integração dos dados e da arquitetura neural consolidou-se em um \textit{pipeline} de treinamento implementado com suporte à aceleração por hardware. As dezenas de milhares de janelas geradas a partir do PTBDB foram submetidas a um particionamento (\textit{split}), reservando 80\% do volume para a fase de treino ativo e 20\% estritamente para a validação, assegurando uma métrica isenta para verificar se a rede não estava apenas memorizando a base.

A otimização dos pesos internos das camadas convolucionais foi conduzida por um otimizador Adam, utilizando uma taxa de aprendizado (\textit{Learning Rate}) inicial de 0.001. A função de custo definida para guiar a retropropagação do erro (\textit{backpropagation}) foi o Erro Quadrático Médio (MSE - \textit{Mean Squared Error}), que calcula matematicamente a diferença amostra por amostra entre o sinal reconstruído pelo Autoencoder e o sinal de referência biologicamente limpo.

Ao final do treinamento (após múltiplas épocas de convergência), os parâmetros aprendidos pelo modelo foram consolidados e exportados em um arquivo binário de pesos (\texttt{.pth}). Desta forma, o custo de processamento é transferido da etapa de aquisição para a inferência em software, permitindo que a plataforma aplique o modelo leve em tempo real sobre o hardware de baixo custo descrito no Bloco 1.
