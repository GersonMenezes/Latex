...
\begin{tikzpicture}[
    ...
    \node [block, right=1.5cm of paciente, fill=orange!10] (front) {\textbf{Bloco 1}\\Front-End Analógico\\(AD620 + OPA2134)};
    \node [block, right=1.5cm of front, fill=green!10] (adc) {\textbf{Bloco 2}\\Conversão e Controle\\(ADS1115 + Arduino)};
    \node [block, below=1.5cm of adc, fill=purple!10] (pc) {\textbf{Blocos 3 e 4}\\Comunicação, IHM e \textit{Deep Learning} (PC)};

    % Desenhando as setas de conexão
    \path [line] (paciente) -- node[above, font=\footnotesize] {Biopotencial} (front);
    \path [line] (front) -- node[above, font=\footnotesize] {Sinal Filtrado} (adc);
    \path [line] (adc) -- node[right, font=\footnotesize] {USB/Serial} (pc);
